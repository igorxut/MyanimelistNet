...
def get_list(f_list, f_user, f_site, f_file_errors, f_file_results):
    ...
    for item_type in ("anime", "manga"):
        ...
        for item in root_xml.iter("series_{0}db_id".format(item_type,)):
            ...
            try:
                ...
                write_log(f_file_errors, log_text)
...
def get_page(f_site, f_task, f_file_errors, f_file_results):
...
def parse_page(f_page, f_file_errors, f_file_results):
...
def main(user):
    ...
    error_log_file = open(file_error_log, "w")
    error_log_file.close()
    result_file = open(file_result, "w")
    result_file.close()

    error_log_file = open(file_error_log, "a")
    result_file = open(file_result, "a")
    ...
    while queue:
        ...
        if task not in user_list:
            ...
            write_log(result_file, log_text)
        ...
        page = get_page(site, task, error_log_file, result_file)

        if page is None:
            continue

        else:
            urls_set = parse_page(page, error_log_file, result_file)
